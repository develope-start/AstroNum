    ...
    <div ref={boxRef} className="relative z-50">
      <div className="flex gap-2.5">
        <div className="relative flex-1">
          <input
            className="w-full rounded-2xl border border-amber-500/25 bg-[#080418] px-4 py-3 text-sm font-semibold text-slate-100 outline-none transition-all placeholder:text-slate-500 focus:border-amber-400 focus:shadow-[0_0_24px_rgba(245,158,11,0.25)] hover:border-amber-500/40"
            value={query}
          ...
          />
          {searching && (
            <Loader2 className="absolute right-3.5 top-3.5 h-4 w-4 animate-spin text-amber-400" />
          )}
        </div>
        <button
          type="button"
          onClick={() => setMapOpen((v) => !v)}
          className="flex shrink-0 items-center gap-1.5 rounded-2xl border border-amber-400/40 bg-gradient-to-r from-amber-500/20 to-purple-600/20 px-4 py-3 text-xs font-bold text-amber-300 shadow-[0_0_15px_rgba(245,158,11,0.2)] transition-all hover:scale-105 hover:border-amber-400"
        >
      ...
      {status === "ok" && value.lat !== null && (
        <div className="mt-2 flex items-center justify-center gap-2 text-xs font-semibold text-amber-300">
        ...
        <div className="absolute left-0 right-0 z-[9999] mt-2 max-h-64 w-full overflow-y-auto rounded-2xl border-2 border-amber-400/60 bg-[#0A041E] p-2 shadow-[0_25px_80px_rgba(0,0,0,0.98)] backdrop-blur-3xl">
          ...
          {localHits.map((h, i) => (
            <button
              ...
              type="button"
              onClick={() => selectHit(h)}
              className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm font-medium text-slate-100 transition-colors hover:bg-amber-500/20 hover:text-amber-300"
            >
              <span>{h.label}</span>
              <span className="text-[0.65rem] font-semibold text-violet-300/80">Asia/Tbilisi</span>
            </button>
          ))}
          {remoteHits.length > 0 && (
          ...
          {remoteHits.map((h, i) => (
            <button
              ...
              className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm font-medium text-slate-100 transition-colors hover:bg-amber-500/20 hover:text-amber-300"
            >
              <span className="truncate max-w-[220px]">{h.label}</span>
              <span className="text-[0.65rem] font-semibold text-violet-300/80">{h.timezone || "მსოფლიო"}</span>
            </button>
          ))}
        </div>
      )}
