...
export default function SynastryCalculator() {
  ...
  return (
    <div className="mx-auto w-full max-w-full space-y-4 sm:space-y-6 text-center overflow-x-hidden">
      ...
      <div className="glass-panel relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl sm:rounded-[28px] p-5 sm:p-6 backdrop-blur-2xl shadow-xl text-center w-full">
        <div className="flex items-center justify-center gap-2.5 text-xs font-bold uppercase tracking-wider text-amber-300">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl border border-rose-500/30 bg-rose-500/15 text-rose-300 shadow-[0_0_18px_rgba(244,63,94,0.35)] shrink-0">
          ...
          <div className="text-left">
            ...
            <span className="text-[0.68rem] text-slate-300 font-medium">ორი ნატალური რუკის შედარებითი ანალიზი</span>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-2.5 sm:gap-3 w-full sm:w-auto">
          <button
            ...
            disabled={loading}
            className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-full btn-gold px-6 sm:px-8 py-3.5 text-xs sm:text-sm font-black disabled:opacity-50 cursor-pointer"
          >
            {loading ? (
              <>
          ...
          {me && (
            <button
              ...
              disabled={loading}
              className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-full border border-amber-400/40 bg-purple-950/40 px-6 py-3.5 text-xs sm:text-sm font-bold text-amber-300 shadow-[0_0_18px_rgba(229,169,59,0.2)] transition-all hover:scale-105 hover:border-amber-400 cursor-pointer"
            >
