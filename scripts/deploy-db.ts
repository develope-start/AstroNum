...
console.log(
  "Existing PostgreSQL schema detected. Applying the idempotent AstroNum baseline before resolving it...",
);

const applyBaseline = runPrisma(["db", "execute", "--file", baselineFile]);
if (applyBaseline.status !== 0) {
  process.exit(applyBaseline.status);
}

const resolveBaseline = runPrisma([
  "migrate",
  "resolve",
  "--applied",
  baselineName,
